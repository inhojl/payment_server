defmodule PaymentServer.Accounts.Multis.SendMoney do
  ...
  defp lock_wallet(user_id, currency) do
    ...
    |> Repo.one()
    |> case do
      nil -> {:error, ErrorMessage.not_found(
  ...
  defp broadcast_transactions_and_extract_sender_wallet(%{
    ...
    {:ok, sender_transaction} = Accounts.create_transaction(
      sender_wallet,
      transaction_amount,
      TransactionType.credit()
    )

    {:ok, recipient_transaction} = Accounts.create_transaction(
      recipient_wallet,
      Decimal.mult(transaction_amount, exchange_rate),
      TransactionType.debit()
    )

    Absinthe.Subscription.publish(PaymentServerWeb.Endpoint,
      sender_transaction,
