defmodule PaymentServer.ExternalApis.AlphaVantage do
  ...
  defp handle_get_exchange_rate(from_currency, to_currency) do
    ...
    case Req.get(url: "http://localhost:4001/query", params: params) do
      ...
      {:ok, %{status: status, body: message} = resp} -> {:error, ErrorMessage.internal_server_error(external_error_message(__MODULE__, __ENV__.function, status, message), resp)}
      {:error, error} when is_exception(error) -> {:error, ErrorMessage.internal_server_error(unknown_error_message(__MODULE__, __ENV__.function), error)}
    end
  end
  ...
  defp handle_fetch_in(body, key_list) do
    case AccessExt.fetch_in(body, key_list) do
      ...
      {:error, key} ->
        ...
        {:error, ErrorMessage.not_found("[#{__MODULE__}.#{function_name}/#{arity}] Failed at key #{key} while traversing body")}
    end
  end
